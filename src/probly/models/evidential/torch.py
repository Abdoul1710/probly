"""Models for evidential deep learning using PyTorch."""  # noqa: INP001
...
class NatPN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        # `self.head` is an nn.Module with a more specific return type; cast
        # so mypy understands we're returning a dict[str, Tensor].
        out = cast(
...
class EvidentialRegressionModel(nn.Module):
    ...
    def __init__(self, encoder: nn.Module | None = None) -> None:
        ...
        self.encoder = encoder
        # EvidentialHead in layers expects a positional/`latent_dim` argument.
        # Use the positional form so mypy recognizes the correct signature.
        self.head: EvidentialHead = EvidentialHead(encoder.feature_dim)

    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Forward pass through encoder and head."""
        features = self.encoder(x)
        # Cast the result of a generic nn.Module call so mypy knows the exact
        # tuple return type produced by EvidentialHead.
        out = cast(
...
class DirichletClassificationModel(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        self.encoder = DirichletMLPEncoder(
            input_dim=input_dim,
            hidden_dim=hidden_dim,
            latent_dim=latent_dim,
        )
        self.head = DirichletHead(latent_dim=latent_dim, num_classes=num_classes)
